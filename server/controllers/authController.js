      ...
      db.query(
        ...
        (err, results) => {
          ...
          QRCode.toDataURL(qrData, (err, qrCodeDataUrl) => {
            ...
            const updateQuery = 'UPDATE customers SET qrcode = ? WHERE customerID = ?';
...
exports.verifyEmail = (req, res) => {
  ...
  db.query("SELECT * FROM customers WHERE verificationToken = ? AND isVerified = 0", [token], (err, results) => {
 ...
 console.log("Token received:", token);
    ...
    if (results === 0){
      ...
      return res.status(500).json({message: 'Invalid or expired verification link'});

    }



    db.query('UPDATE customers SET isVerified = 1, verificationToken = NULL WHERE verificationToken = ?', [token], (err) => {
...
exports.login = (req, res) => {
  ...
  // Find user by email
  db.query('SELECT * FROM customers WHERE email = ? AND isVerified = 1', [email], async (err, results) => {
    if (err) {
...
exports.forgotPassword = (req, res) =>{
  ...
  db.query('SELECT * FROM customers WHERE email = ?', [email], (err, results) => {
    ...
    db.query('UPDATE customers SET resetToken = ?, resetExpires = ? WHERE email = ?', [resetToken, resetExpires, email], (err, updateResults) => {
...
exports.resetPassword = async (req, res) => {
  ...
  db.query('SELECT * FROM customers WHERE resetToken = ? AND resetExpires > NOW()', [token], async (err, results) => {
    ...
    db.query('UPDATE customers SET password = ?, resetToken = NULL, resetExpires = NULL WHERE customerID = ?', [hashedPassword, user.customerID], (err, updateResults) =>{
