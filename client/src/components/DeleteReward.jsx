      ...
      <div className="relative bg-white rounded-lg shadow-lg w-full max-w-md mx-auto p-6">
        ...
        <div className="grid gap-4 py-4">
          <div className="grid grid-cols-4 items-center gap-4">
            <label htmlFor="reward-select" className="text-right text-sm font-medium text-gray-700">
              Select Reward
            </label>
            <select
              id="reward-select"
              ...
              disabled={loading}
            >
              <option value="">Select a reward to delete</option>
              {rewards.map((reward) => (
                <option key={reward.rewardID} value={reward.rewardID}>
                  {reward.rewardName} - {reward.brand} ({reward.pointsRequired} points)
                </option>
              ))}
            </select>
          </div>
          ...
          {loading && (
            <p className="text-gray-500 text-sm col-span-4 text-center">Loading rewards...</p>
          )}

          {selectedReward && (
            <div className="mt-2 p-3 border border-red-200 rounded-md bg-red-50">
              <p className="text-sm font-medium text-red-900">
                ⚠️ This action cannot be undone. The selected reward will be permanently deleted.
              </p>
            </div>
          )}
        </div>
        <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium text-white ${
              !selectedReward ? "bg-gray-400 cursor-not-allowed" : "bg-red-600 hover:bg-red-700"
